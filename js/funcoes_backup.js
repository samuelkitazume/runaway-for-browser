...
function AUXILIARXY(){
		alert('Objetos.x = ' + objetos.x);
		alert('Objetos.y = ' + objetos.y);
		alert('player1.x,y = ' + jogador1.x + ',' + jogador1.y + ' | ComparadorX = ' + jogador1.comparadorX + ' | ComparadorY = ' + jogador1.comparadorY);
		alert('player2.x,y = ' + jogador2.x + ',' + jogador2.y + ' | ComparadorX = ' + jogador2.comparadorX + ' | ComparadorY = ' + jogador2.comparadorY);
	}
...
function Player(jogador,vocation){
	...
	this.comparadorX = false; /* Essas variáveis servirão para comparar coordenadas com objetos no cenário */
	this.comparadorY = false;
	...
	this.changeX = function(a){
		if((a>-2)&&(a<2)){
			if (objetos.x.indexOf(this.x+a)!=-1) { if(!this.comparadorX){this.comparadorX=!this.comparadorX;}} 
			else { if(this.comparadorX){this.comparadorX = !this.comparadorX;}}
			if ((!this.comparadorX)||(!this.comparadorY)){
				this.x = this.x + a; 
	...
	this.changeY = function(b){
		if((b>-2)&&(b<2)){
			if (objetos.y.indexOf(this.y+b)!=-1) { this.comparadorY = true } else { this.comparadorY = false }
			if ((!this.comparadorX)||(!this.comparadorY)){
				this.y = this.y + b;
...
function criaTile (id , classe , x , y , tangivel , content) {
	...
	if (objetos.x.indexOf(player1.x)!=-1) { player1.comparadorX = true; }
	if (objetos.x.indexOf(player2.x)!=-1) { player2.comparadorX = true; }
	if (objetos.y.indexOf(player1.y)!=-1) { player1.comparadorY = true; }
	if (objetos.y.indexOf(player2.y)!=-1) { player2.comparadorY = true; }
...
function deletaTile (id) {
	...
	if (objetos.x.indexOf(x)==-1){
		if(player1.x==x){ player1.comparadorX=false }
		if(player2.x==x){ player2.comparadorX=false }
	}
	if (objetos.y.indexOf(y)==-1){
		if(player1.y==y){ player1.comparadorY=false }
		if(player2.y==y){ player2.comparadorY=false }
	}
	if (objetos.x.indexOf(player1.x)!=-1) { player1.comparadorX = true; } else { player1.comparadorX = false; } 
	if (objetos.x.indexOf(player2.x)!=-1) { player2.comparadorX = true; } else { player2.comparadorX = false; }
 	if (objetos.y.indexOf(player1.y)!=-1) { player1.comparadorY = true; } else { player1.comparadorY = false; }
	if (objetos.y.indexOf(player2.y)!=-1) { player2.comparadorY = true; } else { player2.comparadorY = false; }
}
...
function geraHit(x,y,hit,id,vitima){
	...
	setTimeout(function(){deletaTile(novaId)},10000)
}
...
function movimentacao(e){
	...
	if (((tecla>36)&&(tecla<41))||(tecla==46)||(tecla==34)) {
		switch (tecla){
			...
			case 46: if (verificaChoque()) { fistAtk('jogador1') } break;
	...
	if (String.fromCharCode(tecla).match(/[A-z]/) != null ) {
		switch (String.fromCharCode(tecla)){
			...
			case "Q": if(verificaChoque()) { fistAtk('jogador2') } break;
